<?php
	...
	class SystemVars extends Model {
		
		const _TABLE = 'system_settings';
		private $_db = NULL;
		...
		*/
		...
		public function fetchById($settingid){
			$this->initDb();
			$dataArray = $this->_db->select(self::_TABLE, "variable_key='$settingid'");
			$this->dispose();
		...
		public function get($type){
			$this->initDb();
			$dataArray = $this->_db->select(self::_TABLE, "variable_key='$type'");
			$this->dispose();
			return $dataArray[0]['variable_value'];	
		}
		...
		public function put( $type , $emailText ){
			...
			$this->_db->update(self::_TABLE, array('variable_value'=>$newvalue) , "variable_key='$type'" );
			$this->dispose();
		}
		...
		public function remove($type, $emailtext ){
			...
			if(count($emailArray) > 0){
				...
				$this->_db->update(self::_TABLE, array('variable_value'=>$afterRemoved) , "variable_key='$type'" );	
			}else{
				$this->_db->update(self::_TABLE, array('variable_value'=>'') , "variable_key='$type'" );
			}
			$this->dispose();
		}
